# Write completely correct CameraCard.jsx
...
export default function CameraCard({ camera, onEdit, onDelete }) {
  ...
  const handleRefresh = () => {
    ...
    setStreamUrl("");
    setTimeout(() => setStreamUrl(refreshedUrl), 50);
  };
...
import re
opens = len(re.findall(r'<(\w+)(?:\s|>)', c))
self_closing = len(re.findall(r'<(\w+)\s[^>]*/>', c))
closes = len(re.findall(r'</(\w+)>', c))
print(f'Opening tags: {opens}')
print(f'Self-closing: {self_closing}')
print(f'Closing tags: {closes}')
print(f'Balance: {opens - closes}')

# Check for exact count of key tags
for tag in ['FadeIn', 'Card', 'CardContent', 'div', 'Button', 'p', 'h3', 'img', 'Camera', 'Wifi', 'WifiOff', 'Circle', 'MapPin', 'Maximize2', 'RefreshCw', 'Pause', 'Play', 'Edit3', 'Trash2', 'LivePulse']:
    o = c.count(f'<{tag}')
    if o > 0:
        if tag in ['img', 'Camera', 'Wifi', 'WifiOff', 'Circle', 'MapPin', 'Maximize2', 'RefreshCw', 'Pause', 'Play', 'Edit3', 'Trash2', 'LivePulse']:
            print(f'  {tag}: {o} (self-closing)')
        else:
            co = c.count(f'</{tag}>')
            status = 'OK' if o == co else f'MISMATCH ({o} open, {co} close)'
            print(f'  {tag}: {status}')
